...
const NurseShiftManagement = () => {
  ...
  const onFinish = (values) => {
    const newShift = {
      ...
      nurseName: nurses.find((d) => d.id === values.nurseId)?.name || "",
  ...
  const onFinishBulk = (values) => {
    ...
    if (!nurseIds || nurseIds.length === 0) {
      message.error("Vui lòng chọn Y tá");
      return;
    }
    ...
    while (current.isSameOrBefore(endDate, "day")) {
      if (weekdays.includes(current.day())) {
        shift.forEach((sh) => {
          nurseIds.forEach((id) => {
            const nurse = nurses.find((d) => d.id === id);
            shiftsToAdd.push({
